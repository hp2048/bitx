...
sub get_translation {
	...
	for (my $i=(abs($frame)-1);$i<=length($seq)-3;$i+=3){
		my $thisCodon = substr($seq, $i, 3);
		if (exists $genetic_code{$thisCodon} && $thisCodon =~ /[ACGT]{3}/) {
	   		$translation .= $genetic_code{$thisCodon};
		}
		elsif ($thisCodon =~ /[ACGTN]{3}/) {
			$translation .= "X";
		}
		else {
			print "$thisCodon is not defined in the genetic code.\n.....Aborting the script....\n";
			exit(1);
		}
  	}
	return $translation;
